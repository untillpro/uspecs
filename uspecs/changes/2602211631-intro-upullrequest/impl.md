...
- [x] update: [scripts/_lib/pr.sh](../../u/scripts/_lib/pr.sh)
  - add: `mergedef` command — validates preconditions (git repo, clean tree, not on default branch, not on `--pr` branch) then fetches pr_remote and merges pr_remote/default_branch into change_branch
  - add: `diff` command — accepts `specs` argument; outputs git diff of $specs_folder between current commit and pr_remote/default_branch
...
- [x] create: [u/actn-upr.md](../../u/actn-upr.md)
  - add: Full action file for upr — validation, confirmation, merge step, diff+title/body derivation, PR creation, reporting
