...
#
# Examples:
#   ./deploy.sh --agent claude --uspecs-repo /path/to/uspecs \
#               --marketplace-repo /path/to/uspecs-plugins-claude
#   ./deploy.sh --agent codex --uspecs-repo ... --marketplace-repo ... --release
#   ./deploy.sh --agent augment --uspecs-repo ... --marketplace-repo ... --local
#
# To deploy all three agents, invoke deploy-claude.sh, deploy-augment.sh,
# and deploy-codex.sh in sequence.
...
if command -v cygpath &>/dev/null; then SCRIPT_DIR="$(cygpath -w "$SCRIPT_DIR")"; fi
...
if [ "$RELEASE_MODE" = 1 ] && [ "$LOCAL_MODE" = 0 ] && [ -f "$PLUGIN_JSON" ]; then
  REPO_VERSION="$(python -c "import json,sys; print(json.load(open(sys.argv[1]))['version'])" "$PLUGIN_JSON")"
...
python "$SCRIPT_DIR/gen-uspecs-market.py" \
  --agent "$AGENT" \
