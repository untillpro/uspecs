Feature: Create pull request from current branch
  ...
  Scenario: Create pull request, current branch has a PR associated with it
    Given a PR is associated with the current branch
    When Engineer invokes upr action
    Then message "A PR already exists for the current branch" is displayed
    And PR is opened in the browser
  ...
  Scenario: Create pull request, current branch does not have a PR associated with it
    ...
    # - Prompt is taken using utils.sh#section_templ
    # - Instructions
    #   - Complete PR creation in the browser
    #   - Fix any issues with the PR
    #   - Run `uaccept` once the PR is ready to accept
    #   - Instructions to restore branch to pre-upr state
    #     - `git reset --hard {pre-upr-HEAD}` and `git push --force`
